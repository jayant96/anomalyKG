...
def get_params(dataset):
    pm.params(dataset)
    global ds, name, svm_output_pkl, support_file, tile_plot_file_name, association_plot_file_name, columns_start_count
    name = pm.params.name
    ...
    columns_start_count = pm.params.initial_attribute_count
    ds = 100
    identify_consistent_features()

def identify_consistent_features():
    print("Identifying consistency of the features...")
    df = pd.read_pickle(svm_output_pkl)
    cols_extracted = {}
    for col in df.columns[columns_start_count:-2]:
        print(col)
        two_col_df = df[[col, "svm_binary_output"]].copy()  # Explicitly create a copy
        two_col_df['counter'] = 1
        groups = two_col_df.groupby([col, "svm_binary_output"])['counter'].sum().reset_index()
        FA, FN, TA, TN = 0, 0, 0, 0
...
def main():
    parser = argparse.ArgumentParser(description="Run pattern generation for anomaly detection.")
    parser.add_argument("--dataset", type=int, required=True, help="Dataset ID (e.g., 3 for UMLS)")
    args = parser.parse_args()

    dataset = args.dataset
    get_params(dataset)  # Initialize global parameters
    identify_consistent_features()  # Start the pattern generation process
